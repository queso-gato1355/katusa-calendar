...
import { EventTable } from "@/components/organisms/Admin";
import { useTheme } from "@/components/providers/theme-provider";
import toast from "react-hot-toast";
import { Undo, Trash2 } from "lucide-react";
import { fetchDeletedEvents, restoreEvent, hardDeleteEvent } from "@/lib/api/supabase/helpers";
...
export default function TrashPage() {
  ...
  const handleHardDelete = async (event) => {
    if (
      !window.confirm(
        "정말로 이 일정을 영구적으로 삭제하시겠습니까? 이 작업은 되돌릴 수 없습니다."
      )
    )
      return;
    ...
    try {
      // 일정 영구 삭제
      const { error } = await hardDeleteEvent(event.id);
    ...
    >
      ...
      <div className="md:ml-64 p-4 md:p-8">
        <EventTable
          ...
          onDelete={handleHardDelete} // 휴지통에서는 삭제 기능이 영구 삭제로 대체됨
