...
export async function uploadToStorage(bucket, path, fileContent, options = {}) {
  try {
    const { data, error } = await supabaseAdmin.storage.from(bucket).upload(path, fileContent, {
...
export function getPublicUrl(bucket, path) {
  return supabaseAdmin.storage.from(bucket).getPublicUrl(path)
}
 ...
 */
export async function deleteFromStorage(bucket, paths) {
  try {
