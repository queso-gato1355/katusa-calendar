...
const formatDateToICSDateOnlyOneDayAdded = (date) => {
  // 시간대가 UTC 기준이므로, 대한민국 시간대로 변경
  const d = new Date(date.getTime() + (9 * 60 * 60 * 1000));
  const year = d.getUTCFullYear();
  const month = String(d.getUTCMonth() + 1).padStart(2, "0");
...
export const generateICS = (events, calendarName) => {
  ...
  events.forEach((event) => {
    ...
    if (event.all_day) {
      // 종일이벤트의 경우 종료일은 DB에 저장된 날짜에 하루를 더할 것.
      const startDate = formatDateToICSDateOnly(event.start_at);
      const endDate = formatDateToICSDateOnlyOneDayAdded(event.end_at);
